...
def get_baselines(loader, targets, logger=None):
    df = loader.dataset.datasets[0].cov
    means = df[targets].mean()
    mse = ((df[targets] - means) ** 2).mean()
    print(f"baseline MSEs:\n", mse)
    if logger is not None:
        for target in targets:
            logger.experiment.summary[f"baseline_valid_{target}"] = mse[target]
        logger.experiment.summary[f"baseline_valid_loss"] = mse.sum()
...
logger = None
if WANDB:
    logger = WandbLogger(project="ukb_continuous_prediction")
    params = {
        "epochs": epochs,
        "train_pct": train_pct,
        "targets": targets,
...
get_baselines(vl, targets, logger)
trainer = pl.Trainer(
    gpus=1,
    deterministic=True,
    max_epochs=epochs,
